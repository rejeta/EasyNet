...
- **可靠隧道**：会话级轻量 ARQ（发送窗口、累积 ACK、超时重传），公网丢包场景下仍可维持稳定连接
...
- **主线程**：`poll` 事件循环，处理 UDP/TCP 可读可写、维护 Session 状态、ARQ 重传定时器
- **工作线程**：4 个 Worker 通过任务队列并行处理加密/解密与 I/O，按 `session_id % 4` 哈希分发保证会话有序
- **ARQ 机制**：发送缓存（32 slots）+ 累积 ACK + 400ms 超时重传，重传时重新生成随机 Nonce 加密
...
4. **性能**：当前版本针对 RDP（单会话中高带宽）优化，测试通过 1080p 远程桌面长时间稳定运行
